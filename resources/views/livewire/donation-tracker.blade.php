<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" @if($donation->isPending()) wire:poll.5s="syncStatus" @endif>
    <div class="border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        ...
        @if($donation->isPending())
            <div class="flex items-center gap-1.5 text-xs text-amber-500 font-medium bg-amber-50 px-2.5 py-1 rounded-full animate-pulse">
                <span class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-ping"></span>
                <span>Memantau Pembayaran...</span>
            </div>
        @endif
    </div>
        ...
        <div class="flex items-center gap-3">
            ...
            ">
                ...
                @elseif($donation->isPending())
                    <svg class="w-5 h-5 text-yellow-600 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                @else
