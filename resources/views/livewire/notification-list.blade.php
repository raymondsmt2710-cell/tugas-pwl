<div class="py-8" x-data>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Mark all as read button (fully reactive) --}}
        @if(Auth::user() && Auth::user()->unreadNotifications->count() > 0)
            <div class="flex justify-end mb-4">
                <button type="button" 
                        wire:click="markAllAsRead" 
                        class="text-sm font-medium text-brand-500 hover:text-brand-700 transition">
                    Tandai semua dibaca
                </button>
            </div>
        @endif
        ...
        @else
            <div class="space-y-2 relative" wire:loading.class="opacity-60 pointer-events-none">
                ...
                @foreach($notifications as $notification)
                    <button type="button" 
                            ...
                            class="w-full text-left block rounded-xl border p-4 transition-colors
                                {{ $notification->read_at ? 'bg-white border-gray-100' : 'bg-brand-50 border-brand-100' }}
                                hover:bg-gray-50">
                        <div class="flex items-start gap-3">
                            ...
                            @if(!$notification->read_at)
                                <div class="w-2.5 h-2.5 rounded-full bg-brand-500 shrink-0 mt-2"></div>
                            @endif
                        </div>
                    </button>
                @endforeach
