    ...
    <div class="min-h-screen bg-gradient-to-br from-brand-50 via-white to-brand-100/30 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
            <div class="bg-white py-8 px-6 shadow-xl sm:rounded-3xl border border-gray-100/50">
                ...
                <div class="text-center mb-6">
                    <div class="w-20 h-20 mx-auto rounded-full bg-brand-50 flex items-center justify-center border border-brand-100">
                        <svg class="w-10 h-10 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"/>
                        </svg>
                    </div>
                    <h2 class="mt-5 text-xl font-bold text-gray-900 font-display">Cek Email Anda</h2>
                    <p class="mt-2 text-sm text-gray-500 leading-relaxed">
                        Kami telah mengirim link verifikasi ke
                    </p>
                    <p class="mt-1 text-sm font-semibold text-brand-500">{{ auth()->user()->email }}</p>
                </div>
                ...
                @if (session('status') == 'verification-link-sent')
                    <div class="mb-5 rounded-xl bg-brand-50 border border-brand-200 p-4">
                        <div class="flex items-start gap-3">
                            <svg class="w-5 h-5 text-brand-500 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                            <p class="text-sm text-brand-700 font-medium">Email verifikasi baru berhasil dikirim! Periksa inbox Anda.</p>
                        </div>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('verification.send') }}" id="resend-form">
                    @csrf
                    <button type="submit" id="resend-btn" class="w-full py-3 px-4 rounded-xl bg-brand-500 text-white font-bold text-sm hover:bg-brand-600 shadow-lg shadow-brand-500/10 transition-all hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-brand-500 duration-150 flex items-center justify-center gap-2">
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('logout-to-register') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-500 hover:text-brand-700 transition-colors">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-1.5 text-sm font-semibold text-red-500 hover:text-red-700 transition-colors cursor-pointer">
            ...
            <p class="mt-6 text-center text-xs text-gray-400">
                Butuh bantuan? <a href="mailto:tubespwlkel999@gmail.com" class="text-brand-500 hover:text-brand-700 font-semibold transition-colors">Hubungi kami</a>
            </p>
        </div>
    </div>
</x-guest-layout>
